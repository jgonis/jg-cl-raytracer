...
(defun make-jg-matrix (&optional (rows 4) (columns 4) 
                       &key data)
  (let* ((cols (coerce columns 'fixnum))
    ...
    (make-instance 'jg-matrix
                   ...
                   :data arr)))
(defun make-identity-matrix (&optional (rows 4) (columns 4))
...
(defun make-translation-matrix (x-trans y-trans z-trans)
  (let ((trans-mat (make-identity-matrix)))
...
(defgeneric rotate-x (mat x-radians))
(defmethod rotate-x ((mat jg-matrix) (x-radians number))
  (let* ((x-radians-f (coerce x-radians 'float))
         (rotation-mat (make-x-rotation-matrix x-radians-f)))
    (multiply rotation-mat mat)))

(defgeneric rotate-y (mat y-radians))
(defmethod rotate-y ((mat jg-matrix) (y-radians number))
  (let* ((y-radians-f (coerce y-radians 'float))
         (rotation-mat (make-y-rotation-matrix y-radians-f)))
    (multiply rotation-mat mat)))
(defgeneric rotate-z (mat z-radians))
(defmethod rotate-z ((mat jg-matrix) (z-radians number))
  (let* ((z-radians-f (coerce z-radians 'float))
         (rotation-mat (make-z-rotation-matrix z-radians-f)))
    (multiply rotation-mat mat)))
(defgeneric scale (mat x-scale y-scale z-scale))
(defmethod scale ((mat jg-matrix) 
                  (x-scale number) 
                  (y-scale number)
                  (z-scale number))
  (let* ((x-scale-f (coerce x-scale 'float))
         (y-scale-f (coerce y-scale 'float))
         (z-scale-f (coerce z-scale 'float))
         (scale-mat (make-scaling-matrix x-scale-f
                                         y-scale-f
                                         z-scale-f)))
    (multiply scale-mat mat)))
(defgeneric translate (mat x-trans y-trans z-trans))
(defmethod translate ((mat jg-matrix) 
                      (x-trans number)
                      (y-trans number)
                      (z-trans number))
  (let* ((x-trans-f (coerce x-trans 'float))
         (y-trans-f (coerce y-trans 'float))
         (z-trans-f (coerce z-trans 'float))
         (trans-mat (make-translation-matrix x-trans-f
                                             y-trans-f
                                             z-trans-f)))
    (multiply trans-mat mat)))
...
(defmethod uniform-scale ((vec jg-vec) (scalar number))
